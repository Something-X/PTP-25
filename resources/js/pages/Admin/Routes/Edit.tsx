...
type RouteForm = {
  origin: string;
  destination: string;
  base_price: number | string; // ← kalau form input number, biasanya string
};

export default function Edit({ route: flightRoute }: PageProps<{ route: RouteForm & { id: number } }>) {
  const { data, setData, patch, processing, errors } = useForm<RouteForm>({
